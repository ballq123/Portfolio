...
"""
Portions of the implementation below were ultimately not used in favor of the implementation above, which uses 
trackbars so users can manually tune the webcam and maximize drumstick tip visibility.
"""
...
def readFrames(color, drumPads, testing=False):
    ...
    while True:
        ...
        # Threshold to get only blue/green colors
        if (color == "green"):
            mask = cv2.inRange(hsv_frame, lower_green, upper_green)
        else:
            mask = cv2.inRange(hsv_frame, lower_blue, upper_blue)
        ...
        # tuple argument is the kernel size; bigger kernal size blurs more 
...
# Applies exponentially-weighted moving avg formula to buffer to yield one prediction (ultimately discarded)
alpha = 0.8
def getBufferAvg(color):
    global bufSize
    global alpha
    global bufferBlue, bufferGreen
    global bufIndex

    with bufIndexLock:
        lastBufferIdx = bufIndex
    if color == 'green':
        with greenBufferLock:
            bufCopy = copy.copy(bufferGreen)
    else:
        with blueBufferLock:
            bufCopy = copy.copy(bufferBlue)
    lastAverage = 0
    for i in range(bufSize):
        # Access elements in circular buffer, from least to most recent
        index = (lastBufferIdx + i) % bufSize  
        prediction = bufCopy[index]
        if prediction < 1 or prediction > 4:
            continue  
        lastAverage = (alpha * prediction) + ((1 - alpha) * lastAverage)
    return round(lastAverage)
